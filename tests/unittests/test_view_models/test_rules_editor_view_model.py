...
def test_rules_editor_view_model_update_mf_options(qtbot, rules_editor_view_model, mock_fuzzy_service):
    mock_fuzzy_service.get_input_variables.return_value = [{"name": "input1"}]
    mock_fuzzy_service.get_output_variables.return_value = [{"name": "output1"}]
    mock_fuzzy_service.get_membership_functions.return_value = [{"name": "mf1"}]
