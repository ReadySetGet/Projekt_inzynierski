...
class BellPlot:
    """Support class used to plot bell plot objects in the main window.
    ...
    def __init__(self, plot_widget, x_data, y_data, a_data, b_data, c_data, color):
        """Initialize the bell plot.

        Args:
            plot_widget: The plot widget to display the bell curve.
            x_data: X coordinate data.
            y_data: Y coordinate data.
            a_data: A parameter for the bell curve.
            b_data: B parameter for the bell curve.
            c_data: C parameter for the bell curve.
        ...
        self.position_anchor = pg.TargetItem(pos=(self.c, 0), size=10, symbol="s", pen=self.color, brush=self.color)
    ...
    def _update_b(self, item):
        """Update B anchor position within bounds.

        First checks for whether B anchor is within bounds of its respective A anchor
        and point C. Then it calculates new B for the bell plot formula based on
        the new position.
        """
        new_pos = item.pos()
        new_x = new_pos.x()
