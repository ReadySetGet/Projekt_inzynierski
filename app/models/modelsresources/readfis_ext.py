from fuzzylab import sugfis, mamfis, fisvar, fismf
...
def readfis(filename=''):
    ...
    # 193243: Added try around the functions to catch format errors
...
def get_next_fis_io(fid, i, in_or_out):
    ...
    values = get_line(fid).split('=')
    assert values[
               0] == 'NumMFs', f'Number of MFs for {in_or_out} {i + 1} expected'
...
def get_next_rule(fid, num_inputs):
    ...
    values_ext = [val for val in re.split(r';', line_ext) if val]
    values_is_mf = [val for val in re.split(r':|,|\(|\)| ',
                                            values_ext[1]) if val]
    for i in range(len(values_is_mf)):
        values_is_mf[i] = int(values_is_mf[i])

    rule_name = values_ext[2][1:]

    values = [val for val in re.split(r':|,|\(|\)| ', values_ext[0]) if val]

    # 193243: end of changes

    for i in range(len(values) - 2):
        values[i] = int(values[i])

    values[-2] = int(values[-2]) if (
        float(values[-2])).is_integer() else float(values[-2])
    values[-1] = int(values[-1])

    # 193243: changed to FisRuleEx, values in additional brackets
