...
def write_input_sections(fid, fis):
    ...
    for i in range(num_inputs):
        ...
        for j in range(num_mfs):
            ...
            fid.write(
                f"MF{j + 1}='{mf.Name}':'{mf.Type}',{str(mf.Parameters).replace(',', '')}\n")
...
def write_rules_section(fid, fis):
    ...
    for i in range(num_rules):
        ...
        next_is_mf = fis.Rules[i].IsMFInput + fis.Rules[i].IsMFOutput
        next_name = fis.Rules[i].Name
        ...
        # 193243: Old code:
        # 193243: fid.write(f"{next_connect}\n")

        # 193243: Changes:

        fid.write(f"{next_connect} ; ")

        for j in range(len(next_is_mf) - 1):
            fid.write(f"{next_is_mf[j]} ")

        fid.write(f"{next_is_mf[-1]} ; ")
